...
$struniquereply = get_string('uniquereply', 'block_forum_report');
//BL Customized Code <<--
if (!$startnow) {
    echo '<br>';
    ...
    $table->define_columns(array('fullname', 'group', 'country', 'institution', 'posts', 'replies', 'wordcount', 'views', 'multimedia', 'uniqueviewdays', 'uniquereplydays', 'firstpost', 'lastpost', 'action'));
    $table->define_headers(array($strname, $strgroup, $strcounrty, $strinstituion, $strposts, $strreplies, $strwordcount, $strviews, $strmultimedia, $struniqueview, $struniquereply, $strfp, $strlp, ''));
    $table->sortable(true);
    ...
    foreach ($students as $student) {
        ...
        if ($postreplies) {
            $studentdata->uniquepostreplydays = count(array_unique($uniquepostreply));
        } else {
            $studentdata->uniquepostreplydays = "0";
        }
        //BL Customized Code <<--
        //View
    ...
    foreach ($data as $row) {
        ...
        $trdata = array($row->name, $row->group, $row->country, $row->institution, $row->posts, $row->replies, $row->wordcount, $row->views, $row->multimedia, $row->uniqueviewdays, $row->uniquepostreplydays, $row->firstpost, $row->lastpost, $sendreminder . $complink);
